...
exports.printIncompleteItems = printIncompleteItems;
...
const getDirs = exports.getDirs = dir => _fsExtra2.default.readdirSync(dir).filter(f => _fsExtra2.default.statSync(dir + "/" + f).isDirectory());

function printIncompleteItems(itemsArry) {
...
function makeNote(notesData) {
  ...
  notesData.forEach(section => {
    ...
    const Header = `# ${section.title}: ${cliRef} --> ( ${completedItems} / ${itemCount} )\n`;
  ...
  _fsExtra2.default.readJson(configPath, (err, configJSON) => {
    ...
    _fsExtra2.default.readJson(dataPath, (err, dataJSON) => {
      ...
      const filteredDataJSON = dataJSON.filter(note => {
        ...
        if (!keepSection) {
          console.log(`"${_chalk2.default.cyan(note.title)}" section was removed!`);
        }
        return keepSection;
      });

      _fsExtra2.default.writeJson(dataPath, filteredDataJSON, err => {
        if (err) return console.error(err);
